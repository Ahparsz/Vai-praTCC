                        ...
                        <a href='logout.php' class='btn btn-md btn-danger float-right'>Sair</a>
                            <?php
                                ...
                                if (!isset($_SESSION['usuario']) OR ($_SESSION['nivel'] > $nivel_necessario)) {
                                    echo "<a class='btn btn-success float-left' href='restrito.php'>Administrar</a>";
                                }
                            ?>
                    </div>
                </div>
                                    ...
                                    $show = "SELECT * FROM favorito, cidade WHERE favorito.id_usuario=".$usuario." AND cidade.cd_cidade=favorito.id_cidade";
                                            if ($query = $mysqli->query($show)) {
                                                while ($dados = $query->fetch_object()) {
                                                    ...
                                                    echo "
                                                        <div class='col-6'>
                                                            <h4><a href=cidades.php?codigo_filtro=".$_SESSION['cd_cidade'].">".$dados->nm_cidade."</a></h4>
                                                        </div>
                                                        
                                                        <div class='col-6'>
                                                            <a href=excluir.php?codigo_favorito=$dados->cd_favorito>
                                                            <img src='img/remover.png' width='35px' heigth='35px'></a><br>
                                                        </div>
