...
?>
    ...
    <div class='container-fluid'>
        <div class='row'>
            ...
            <div class='col-sm-5 bg-white'id='caixas'>    
            <?php
            //FOTO DA REGIAO
                if(isset($_POST['regiao'])) {
                    $regiao = $_POST['regiao'];
                    ...
                    $sql = "SELECT * FROM regiao WHERE regiao.cd_regiao =".$regiao;
                        if($query = $mysqli->query($sql)){
                            if($query->num_rows>0){
                                while($dados = $query->fetch_object()){
                                    echo "<img class='justify' width='470px' height='240px' src='img/regiao/".$dados->cd_regiao.".jpg'>";
                                }
                            }
                        }
                }
                ...
                <div data-spy="scroll" data-target="#navbar-vertical" data-offset="0" class='scrollspysite'>
                    ...
                    <label>
                        ...
                        if($query = $mysqli->query($clima)){
                            if($query->num_rows>0){
                                while($dados = $query->fetch_object()){
                                    echo "<input type='checkbox' class='clima' name='clim' value='$dados->cd_clima'> $dados->nm_clima <br>";;
                                }
                            }
                        }
						...
						if ($result = $mysqli->query($ambientesql)) {
							...
							while($obj = $result->fetch_object()){
								echo "<input type='checkbox' class='ambiente' name='ambiente' value='$obj->cd_ambiente'> $obj->nm_ambiente <br>";
							}
						} 
						...
						if ($result = $mysqli->query($tiposql)) {
							...
							while($obj = $result->fetch_object()){
								echo "<input type='checkbox' class='tipo' name='tipo' value='$obj->cd_tipo'> $obj->nm_tipo <br>";
							}
						} 
